...
export class Coupon {
  ...
  /** 券父ID */
  @Column({ length: 24, comment: '卡券父Id' })
  @IsString()
  // 券ID关联的父ID
  pId: string

  /** 券ID */
  @Column({ length: 24, unique: true, comment: '卡券Id, 唯一键' })
  @IsNotEmpty()
  @IsString()
  // 自动生成唯一的 UUID
  ...
  /** 用户ID */
  @Column({ length: 24, comment: '用户ID' })
  @IsString()
  userNo: string

  /** 券码 */
  @Column({ length: 24, comment: '卡券券码' })
  @IsNotEmpty()
  @IsString()
  code: string
  ...
  /** 最近更新时间 */
  @Column({ type: 'timestamp', comment: '最近更新时间' })
  updateAt: Date
  ...
  /** 卡券描述 */
  @Column({ length: 300, comment: '卡券描述' })
  description: string
